**Protocolo v1.3 — Ações vs CDI (simulação educacional reprodutível)**

Versão: 1.3  
Data de congelamento: 14/02/2026  
...
Regra de mudanças: qualquer alteração após este congelamento vira versão 1.4 e exige justificativa escrita (o que mudou, por quê, impacto esperado).

Changelog:  
v1.1 — Substituição por auditabilidade (pré-coleta): KLBN3 apresentou histórico mensal incompleto no Mais Retorno para recortes exigidos, reduzindo reprodutibilidade/QA. Substituída por GGBR3 no Portfólio B, mantendo a regra setorial (economia real: indústria/metais). Demais parâmetros inalterados.  
v1.2 — Substituição por auditabilidade (pré-coleta): constatou-se que GGBR3 também não possui histórico mensal completo no Mais Retorno para recortes exigidos (ex.: D-12m 10/02/2020→10/02/2025 indica Qtd.M=288, abaixo do esperado), reduzindo reprodutibilidade/QA. Substituição: GGBR3 → ALPA3 no Portfólio B, mantendo a regra setorial (economia real). Demais parâmetros inalterados.  
v1.3 — Errata (correção editorial; sem impacto analítico). Após o registro do Protocolo v1.2, foram identificados dois erros redacionais: (i) a seção “Universo e elegibilidade” ainda listava KLBN3 e GGBR3 como parte do universo, apesar de ambas terem sido substituídas por auditabilidade (KLBN3→GGBR3 no v1.1; GGBR3→ALPA3 no v1.2); e (ii) a contagem do “universo core (15)” estava incorreta. O universo efetivo do estudo, definido como a união dos tickers presentes nas carteiras A/B/C congeladas, é de 14 empresas. Esta versão v1.3 corrige apenas texto/contagem para consistência interna, sem alterar carteiras, janelas, benchmarks, métricas, robustez, regras de QA ou plano de análise.
...
\- Qualquer mudança depois do congelamento → Protocolo v1.4 com justificativa escrita.
...
Universo efetivo do estudo (definição operacional):  
O universo efetivo do estudo é definido como a união dos tickers presentes nas carteiras A/B/C congeladas. Com as carteiras vigentes, esse universo é composto por 14 empresas (tickers atuais):  
BBDC3, BBAS3, ITUB3, PETR3, VALE3, CMIG3, CPLE3, EGIE3, VIVT3, SBSP3, ABEV3, LEVE3, CSNA3, ALPA3.
...
Notas de auditabilidade (pré-coleta):  
KLBN3 e GGBR3 foram excluídas/substituídas por auditabilidade no Mais Retorno, conforme changelog (v1.1 e v1.2), para preservar reprodutibilidade e QA.

Trade-off explícito:  
Priorizar auditabilidade/reprodutibilidade em detrimento de abrangência máxima do universo.
...
\- Indústria/metais: CSNA3  
\- Consumo cíclico/industrial: ALPA3
...
As carteiras A/B/C listadas abaixo são a instanciação fixa desta regra para este estudo, congelada na versão 1.3; qualquer alteração de ativos, pesos, número de ações, ou dos blocos setoriais definidos acima exige versão 1.4 com justificativa explícita.

Carteiras (congeladas):  
\- Portfólio A: PETR3, VALE3, ITUB3, BBAS3, CMIG3, EGIE3, ABEV3, VIVT3  
\- Portfólio B: PETR3, VALE3, BBDC3, ITUB3, CPLE3, EGIE3, ALPA3, SBSP3  
...
\- Consistência matemática: retorno acumulado vs encadeamento de retornos periódicos (quando disponível).
...
\- Não expandir universo além do universo efetivo definido pelas carteiras (a menos que vire v1.4).  
...
Assinatura do protocolo (v1.3)

Autor: Patrick Gerard Pin —  Data: 14/02/2026  
Declaração de congelamento: “Qualquer mudança depois disso é versão 1.4 e precisa justificativa.”
